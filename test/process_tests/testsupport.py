...
def project_dirs() -> List[Path]:
    dirs = [
        ...
        PROJECT_ROOT.joinpath("build", ""),
        # Add project build dev dir to PATH
        PROJECT_ROOT.joinpath("build", "dev"),
...
def find_project_executable(exe: str) -> str:
    ...
    raise OSError(
        f"executable '{exe}' not found. The following locations were considered:\n  {locations}"
    )
